...
foreach ($DarrayMonth as list($month_value, $year_value)) {
    ...
    if ($arrayCharges[$i]=="disabled") {
        ...
        $list=''; // Initialisation
        $list.='<td><input type="number" step="any" min="0" name=taskIntAffPlanMonth-'.$month_value.'-'.$year_value.' value=0'; 
        ...
        $list=''; // Initialisation
        $list.='<td><input type="number" step="any" min="0" name=taskIntAffRealMonth-'.$month_value.'-'.$year_value.' value=0';
        if($year_value>date("Y") or ($year_value==date("Y") and $month_value>date("n"))){$list.=" disabled class=inputDisabled";}
        $list.='></td>';
        array_push($listValues,$list);
